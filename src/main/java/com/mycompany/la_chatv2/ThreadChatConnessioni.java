...
public class ThreadChatConnessioni implements Runnable
{
  ...
  public ThreadChatConnessioni(ThreadGestioneServizioChat gestoreChat,Socket client)
  {
    ...
    try
    {
      this.input = new BufferedReader(new InputStreamReader(client.getInputStream())); //stream i
      this.output = new PrintWriter(this.client.getOutputStream(),true);    //stream o
    }catch(Exception e){
